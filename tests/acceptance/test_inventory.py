...
echo LSB OS""",
        ...
        for file in [src['name'] for src in sources]:
            connection.run("if [ -e %s ]; then mv %s %s.backup; fi" % (file, file, file))
        ...
        try:
            ...
            for src in sources:
                ...
                if src.get('name') is not None:
                    connection.run("rm -f %s" % src['name'])
        finally:
            for file in [src['name'] for src in sources]:
                connection.run("if [ -e %s.backup ]; then mv %s.backup %s; fi" % (file, file, file))
